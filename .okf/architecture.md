...
7. The bundled Pi extension sends a POST on `agent_end`; the bridge creates an IDE `Pi needs attention` notification unless the Pi tab is selected.
...
- Suppress/dismiss attention notifications while the Pi tab is selected.
